...
module ucmd_fsm #( 
    parameter ucmd_g             = $1, 
    parameter ucmd_fbs           = $2, 
    parameter ucmd_adr           = $3, 
    parameter ucmd_c             = $4, 
    parameter cmd_c              = $5,
    parameter ucmdRAM_data_w     = $M,
    parameter ucmdRAM_adr_w      = $N,
    parameter ucmdARAM_data_w    = $O,
    parameter ucmdARAM_adr_w     = $P,
    ...
    generate 
        if (control_type=="carry4") begin
            ...
            ucmd_carry4 #(.ucmd_g(ucmd_g), .ucmd_adr(ucmd_adr), .ucmd_fbs(ucmd_fbs)) ucmd_carry4_$0_inst (
...
module bitwise_logic_x8_with_carry4 #(parameter data_w = 32) (
  ...
  carry4_chain #(extended_data_w/2) u_carry4_chain_$0 (.data_in(f_ext), .carry_in(en), .result(result));
...
module ucmd_carry4 #(parameter ucmd_g = 2, parameter ucmd_adr = 32, parameter ucmd_fbs = 32) (
  ...
  generate
    for (n = 0; n <= ucmd_adr; n = n+1) begin: carry4_chain_gen
		carry4_chain #(extended_ucmd_term_en3_width) u_carry4_chain_$0 ( // Используем carry4_chain
...
`ifdef chipscope
	...
	assign cmd = vio_port[$2:1];
	assign fbs = vio_port[$5+$2:$2+1];
...
$LSignalExport.triggerPort<0000>.name=trig0
...
$QSignalExport.syncOutputWidth=$R
