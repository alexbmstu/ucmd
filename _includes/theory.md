...
- Строит **граф переходов**, где:
  - **Узлы** — это состояния автомата (например, `IDLE0`, `EX_0`, `H`).
  - **Ребра** — это переходы между состояниями, которые зависят от условий (например, `@IF (feedback<0>=1) => IDLE0`).
