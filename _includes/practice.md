...
<img src="assets/uart_hello_world.png" alt="Функциональная схема устройства" style="width:800px;"/>
...
<img src="data/ucmd/ucmd_dest/uart_hello_world.svg" alt="Диаграмма переходов состояний микропрограммного автомата Hello world!"/>
...
1. **Регулярное выражение**: `([a-zA-Z]{2,}[0-9]{1,}){2,}`  
   **Пример строки**: `abc123xyz456`  
   (Последовательность букв и цифр, повторенная два или более раза.)

2. **Регулярное выражение**: `([A-Z][a-z]+[0-9]{2}){3,}`  
   **Пример строки**: `Hello42World99Test01`  
   (Заглавная буква, строчные буквы и две цифры, повторенные три или более раза.)

3. **Регулярное выражение**: `[a-fA-F0-9]{4,}([.,:;!?'"]{1}[a-zA-Z]+){2,}`  
   **Пример строки**: `1A3F,abc!xyz`  
   (Шестнадцатеричное число, за которым следуют базовые знаки препинания и буквы, повторенные два или более раза.)

4. **Регулярное выражение**: `([#$%@&]{1}[0-9]+){3,}`  
   **Пример строки**: `#123$456%789`  
   (Финансовые знаки препинания, за которыми следуют цифры, повторенные три или более раза.)

5. **Регулярное выражение**: `([()\[\]]{1}[a-zA-Z]+){2,}`  
   **Пример строки**: `(abc)[xyz]`  
   (Скобки, за которыми следуют буквы, повторенные два или более раза.)

6. **Регулярное выражение**: `[{}]{1}[0-9]+([+\-*/\\=<>]{1}[a-zA-Z]+){2,}`  
   **Пример строки**: `{123}+abc*xyz`  
   (Фигурные скобки, за которыми следуют цифры, а затем математические символы и буквы, повторенные два или более раза.)

7. **Регулярное выражение**: `(\s{1}[a-zA-Z]+){3,}`  
   **Пример строки**: ` abc def ghi`  
   (Пробельные символы, за которыми следуют буквы, повторенные три или более раза.)

8. **Регулярное выражение**: `([aeiouAEIOU]{1}[0-9]+){2,}`  
   **Пример строки**: `a1e2i3`  
   (Гласные буквы, за которыми следуют цифры, повторенные два или более раза.)

9. **Регулярное выражение**: `(\0{1}[a-zA-Z]+){2,}`  
   **Пример строки**: `\0abc\0xyz`  
   (Нулевые символы, за которыми следуют буквы, повторенные два или более раза.)

10. **Регулярное выражение**: `([a-zA-Z]+[.,:;!?'"]{1}){3,}`  
    **Пример строки**: `abc,xyz!test:`  
    (Буквы, за которыми следуют базовые знаки препинания, повторенные три или более раза.)

11. **Регулярное выражение**: `([0-9]+[#$%@&]{1}){2,}`  
    **Пример строки**: `123#456$`  
    (Цифры, за которыми следуют финансовые знаки препинания, повторенные два или более раза.)

12. **Регулярное выражение**: `([a-zA-Z]+[()\[\]]{1}){3,}`  
    **Пример строки**: `abc(xyz)[test]`  
    (Буквы, за которыми следуют скобки, повторенные три или более раза.)

13. **Регулярное выражение**: `([0-9]+[{}]{1}){2,}`  
    **Пример строки**: `123{}456{}`  
    (Цифры, за которыми следуют фигурные скобки, повторенные два или более раза.)

14. **Регулярное выражение**: `([a-zA-Z]+[+\-*/\\=<>]{1}){3,}`  
    **Пример строки**: `abc+xyz=test`  
    (Буквы, за которыми следуют математические символы, повторенные три или более раза.)

15. **Регулярное выражение**: `([0-9]+\s{1}){2,}`  
    **Пример строки**: `123 456 `  
    (Цифры, за которыми следуют пробельные символы, повторенные два или более раза.)

16. **Регулярное выражение**: `([a-zA-Z]+[aeiouAEIOU]{1}){3,}`  
    **Пример строки**: `abcAxyzEtestI`  
    (Буквы, за которыми следуют гласные буквы, повторенные три или более раза.)

17. **Регулярное выражение**: `([a-zA-Z]{2,}[0-9]{1,}[.,:;!?'"]{1}){2,}`  
    **Пример строки**: `abc123,xyz456!`  
    (Буквы, цифры и базовые знаки препинания, повторенные два или более раза.)

18. **Регулярное выражение**: `([A-Z][a-z]+[0-9]{2}[#$%@&]{1}){2,}`  
    **Пример строки**: `Hello42#World99$`  
    (Заглавные буквы, строчные буквы, цифры и финансовые знаки препинания, повторенные два или более раза.)

19. **Регулярное выражение**: `([a-fA-F0-9]{4,}[()\[\]]{1}[a-zA-Z]+){2,}`  
    **Пример строки**: `1A3F(abc)5B6D[xyz]`  
    (Шестнадцатеричные числа, скобки и буквы, повторенные два или более раза.)

20. **Регулярное выражение**: `([{}]{1}[0-9]+[+\-*/\\=<>]{1}[a-zA-Z]+){2,}`  
    **Пример строки**: `{123}+abc*xyz`  
    (Фигурные скобки, цифры, математические символы и буквы, повторенные два или более раза.)

21. **Регулярное выражение**: `(\s{1}[a-zA-Z]+[aeiouAEIOU]{1}){3,}`  
    **Пример строки**: ` abcA defE ghiI`  
    (Пробельные символы, буквы и гласные буквы, повторенные три или более раза.)

22. **Регулярное выражение**: `(\0{1}[a-zA-Z]+[0-9]+){2,}`  
    **Пример строки**: `\0abc123\0xyz456`  
    (Нулевые символы, буквы и цифры, повторенные два или более раза.)

23. **Регулярное выражение**: `([a-zA-Z]+[.,:;!?'"]{1}[0-9]+){2,}`  
    **Пример строки**: `abc,123xyz!456`  
    (Буквы, базовые знаки препинания и цифры, повторенные два или более раза.)

24. **Регулярное выражение**: `([0-9]+[#$%@&]{1}[a-zA-Z]+){2,}`  
    **Пример строки**: `123#abc456$xyz`  
    (Цифры, финансовые знаки препинания и буквы, повторенные два или более раза.)

25. **Регулярное выражение**: `([a-zA-Z]+[()\[\]]{1}[0-9]+){2,}`  
    **Пример строки**: `abc(123)xyz[456]`  
    (Буквы, скобки и цифры, повторенные два или более раза.)

26. **Регулярное выражение**: `([0-9]+[{}]{1}[a-zA-Z]+){2,}`  
    **Пример строки**: `123{}abc456{}xyz`  
    (Цифры, фигурные скобки и буквы, повторенные два или более раза.)

27. **Регулярное выражение**: `([a-zA-Z]+[+\-*/\\=<>]{1}[0-9]+){2,}`  
    **Пример строки**: `abc+123xyz=456`  
    (Буквы, математические символы и цифры, повторенные два или более раза.)

28. **Регулярное выражение**: `([0-9]+\s{1}[a-zA-Z]+){2,}`  
    **Пример строки**: `123 abc456 xyz`  
    (Цифры, пробельные символы и буквы, повторенные два или более раза.)

29. **Регулярное выражение**: `([a-zA-Z]+[aeiouAEIOU]{1}[0-9]+){2,}`  
    **Пример строки**: `abcA123xyzE456`  
    (Буквы, гласные буквы и цифры, повторенные два или более раза.)

30. **Регулярное выражение**: `([a-zA-Z]{2,}[0-9]{1,}[.,:;!?'"]{1}[#$%@&]{1}){2,}`  
    **Пример строки**: `abc123,#xyz456!$`  
    (Буквы, цифры, базовые знаки препинания и финансовые знаки препинания, повторенные два или более раза.)
